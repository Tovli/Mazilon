...
import 'package:mazilon/notification_api.dart';
...
TimeOfDay calculateTime(int h, int m) {
  TimeOfDay time = TimeOfDay(hour: h, minute: m);
  print(time);
...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    ...
    var calculatedTime =
        calculateTime(inputData?["timeHour"], inputData?["timeMinute"]);
    var id = inputData?["NotificationId"];
    var quotes = inputData?["text"];
    NotificationsService.cancelNotifications(int.parse(id));
    await NotificationsService.schedulNotification(
