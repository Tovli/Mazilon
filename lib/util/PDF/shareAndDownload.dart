...
Future<Map<String, dynamic>> getPrefsDataForPDF() async {
...
List<List<String>> filterEmptyData(List<List<String>> data) {
...
Future<pw.Document> createPDF(List<dynamic> titles, List<dynamic> subTitles,
  ...
  final dataForPDF = await getPrefsDataForPDF();
  ...
  realData = filterEmptyData(
      [difficultEvents, makeSafer, feelBetter, distractions, phoneDescription]);
...
Future<void> generateAndSharePdf(String message, List<dynamic> titles,
  ...
  try {
    // Add the generated widgets to the PDF
    final pdf = await createPDF(titles, subTitles, texts);
